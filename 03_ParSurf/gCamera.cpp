...
{
	SetView( glm::vec3(0,5,5), glm::vec3(0,0,0), glm::vec3(0,1,0));
...
void gCamera::Update(float _deltaTime)
{
  ...
  //UpdateUVk(m_goFw*m_speed*_deltaTime, m_goRight*m_speed*_deltaTime);

	m_viewMatrix = glm::lookAt( m_eye, m_at, m_up);
	m_matViewProj = m_matProj * m_viewMatrix;
}

void gCamera::UpdateUVk(float du, float dv)
{
	m_uk		+= du;
	m_vk		= glm::clamp<float>(m_vk + dv, 0.1f, 3.1f);
  //m_v += dv;

  m_at = glm::vec3(	cosf(m_uk)*sinf(m_vk), 
										cosf(m_vk), 
										sinf(m_uk)*sinf(m_vk) );
  m_eye = m_at - 
